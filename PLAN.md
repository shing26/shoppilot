...
| 9/11 追加（ticket 20） | S7 之后的判据收口 | 重标 4 条 gold 的合格答案集 + 修评分器四处缺陷，零模型额度、零阈值改动 |
...
`工具调用准确率` 那一行的**量具**口径在 2026-09-11 修过一次，前后两套读数与理由见 ADR 0021：参数子指标只在"合格工具真的打出去、且这一轮拿得到模型实发参数"的样本上算，未比对的条数与比率一并报出。判据本身（分意图各 `>= 95%`、任一意图 `< 80%` 判不通过并写明原因）一字未动，动的是量具，改完仍判未达成。
...
| 20 | 对 09-10 那 6 份 dev 明细跑 `--rescore` | 新旧 `tool_ok` 的差异集合恰好等于放开的那 4 条 id，其余 176 条的 `tool_ok` 逐格一致；本票 24 条验收逐条留证据 |

## 已知限制（写进 README，不藏）

H2 内嵌库在写密集路径上是瓶颈；跨实例 singleflight 已实现但只在单实例环境验证过；身份提供方是 mock 的（验签真、发 token 假）；政策语料为生成数据非真实平台条款；ES 停在够用级、不做 rerank，理由是无精排价值，不是没时间。**（ticket 17 实测新增）** 0.95 工作点上 L2 语义缓存召回实测为 0：bge-m3 在短中文问句上的同意图改写对最大余弦 0.9435，够不到阈值。80% 级热点拦截由 L1 精确哈希与 SingleFlight 承担，L2 只兜"同一政策、写法极近"这一小撮；同桶反义（实测 0.9682 越界一对）由 ADR 0016 的极性守卫兜，不由阈值兜。
