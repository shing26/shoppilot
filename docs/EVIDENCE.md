...
| 262 条 JVM 测试与 Linux 干净 runner | ticket 32、ticket 33、ticket 34、ticket 35、ticket 36、ticket 37、ticket 38、ticket 41、风格票、README badge | `.github/workflows/ci-subset.yml`、GitHub Actions `ci-subset` run `35328032251`（`8e6437e`，50 s）与 run `35350399414`（`8b16a98`）、run `35430345281`（`791fdb0`，票 41 后 228 绿）、run `35440613321`（`a0047fe`，票 34 两步评测门禁首绿）、`GatewayMainPathJvmTest` | 本地 `.\mvnw.cmd -B -ntp verify`；CI `bash ./mvnw -B -ntp verify` | 2026-09-18 新增 3 条网关主链路 JVM smoke（当时 `3 + 12 + 209 = 224`）；2026-09-19 票 41 新增 4 条工具循环语义用例（228 绿）；票 35 新增 5 条 Prompt 版本化用例（233 绿）；票 36 新增 7 条情绪门用例（240 绿）；票 37 新增 9 条反馈账本与复核队列用例（249 绿）；票 38 新增 6 条渠道入站契约用例（255 绿）；风格票新增 7 条档位矩阵与注入拼装用例，本地 `3 + 15 + 244 = 262` 绿；票 34 起 CI 另含两步 0 token 评测门禁（判据自检 40 项断言 + rescore 比对，`RESCORE DONE cases=180 files=6 tool_diff=4` 钉基线）；CI 不覆盖 17 步活体验收、Docker、Ollama、在线评测或压测 |
