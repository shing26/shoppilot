...
| 17 | 工单跨重启持久化 | 工单库仍在业务 Mock 进程内；看门狗 reseed 会抹掉。重开触发 = biz-mock 被真工单系统替换（ADR 0030 第 2 条） | 随真工单系统替换 |
| 18 | RESOLVED→买家回流 | 工单可结单，但结单后没有回到买家的消费路径。与第 17 项共享 ADR 0030 第 2 条触发条件 | 随真工单系统替换 |
