...
10. 第四轮双轴审查（09-12 00:1x，fixed point `9eede6d`；Standards 轴 6 条 + Spec 轴 5 条）。**账按两份报告原文与磁盘重核，不抄上一轮的收口笔记**：
  ...
  ⑫ **收口时又抓到自家量具与自述五处假绿，全在审计脚本与文档自身，不是审查轴报的**：
    ...
    **本轮读数**：收口审计本轮实跑 **88 项机器断言**（项数以 `.scratch/shoppilot-mvp/round3-closeout-audit.txt` 末行为准），
    审计期间 `tokensUsedToday` 前后均 **0**（`llmMode=local`、日预算 260000），全程零模型额度。
    写法口径由 H7/H7b/H12 三面钉：「88 项机器断言」是本轮实数、必须被文档至少引用一次，且入仓产物末行必须等于它；
    上面第 9 条那个 54 是**上一版跑器**的历史数，
    本条上面那几版的 70 / 74 / 75 / 77 / 87 均为**同一支跑器在历轮内递进时的历史数**（70 是 N 算早了，见第 ⑫(b) 项；74 与 75 是后来又加了 H10（钉替换处数）与 H11/H11b（钉圆号子项不重复、引用有落点）；77 是第四轮收口那一版的实数，第五轮双轴审查后加 A3b/B7/E4b/F2c/H1d/H3b/H12/H13/H13b/J0 十项让位给 87；87 又是把 F1 按可核面拆成 F1/F1c 之前的历史数）；
    凡不等于本轮实数的项数，所在行必须带历史标记，不允许裸着当现状。

11. 第五轮双轴审查（09-12 上午，fixed point `a6ccdcb`；Standards 轴 6 条 + Spec 轴 5 条）。**账按两份报告原文与磁盘重核**：
    合计报 11 条，逐条对盘重核后**成立 11 条、驳回 0 条**（9 条硬违规 + 2 条 judgement）。上一版收口笔记把这笔账记成
    「成立 11 条、驳回 3 条」，6+5=11 与「成立 11」同数、那 3 条驳回却无处安放——**第六次**发现上一轮记的账对不上盘，
    且这次的账仍是上一轮的自己记的（幻影驳回那两条见第 10 条末尾）。计划 P10 来源句已按「报 N = 成立 M + 驳回 K」算平。
    落地面全在量具脚本与文档自述上：**审计新增十项断言** A3b / B7 / E4b / F2c / H1d / H3b / H12 / H13 / H13b / J0，
    ...
    ⑥ **Standards F5（硬违规）**：P10 判据第 3 条称 `scripts/`、`eval/` 零改动「由审计 B1-B6 与 F4 钉」，而 B6 白名单明列 `scripts/up.ps1`、
       ...
       处置：新增 B7——窗口 `a6ccdcb..HEAD`、白名单严格到 `README.md` + `.scratch/shoppilot-mvp/`，`scripts/`、`eval/`、`src/`、`docs/adr/` 出现即红；
       P10 那句引用改指 B7（改的是引用，判据本体一字未动）。
    ...
    ⑧ **Spec 第 3 条**：P10 判据第 5 条要求「三条驳回项各留机器反证」，实际只有 H3 那一格有产物。处置：判据本体一字不动，
       ...
       补 H13b 钉「被驳回的那处 `up.ps1` 第 115 行引用在四份文档里确实零命中」；第三条（`logs/` 无痕）由 G 组与 H1 承担，在本条点名指认，不另造尺子。
