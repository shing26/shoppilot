...
11. `logs/acceptance/eval.log` 首行 `SCORER SELFCHECK ok=16`，末行 `EVAL DONE cases=24 errors=0 mode=local limit=24`（同一份日志两条都在）。跨轮对照按**逐行逐列按字段比**做：`tool-eval-20260911-185410-local-smoke-summary.csv` 对 `...-123109-local-smoke-summary.csv`，10 行 × 13 列 = 130 格里 **1 格不同**——`ESCALATE` 的 `overall_accuracy` 50.0% -> 100.0%，明细里对应 `ESC-02` 的 `intent_actual` UNKNOWN -> ESCALATE、`escalate_ok` False -> True（local 模式 3B 模型的抖动，量的是评测链路通不通）。四个 ACTION 行两轮相同：`args_comparable` 3/3、`ACTION_ADDRESS` 的 `args_accuracy` 66.7%。**这一条曾经写错过**：我第一版用 `Compare-Object` 比 `Import-Csv` 的对象，而 `Compare-Object` 对 PSCustomObject 只比 `ToString()`（就是类型名），于是把 130 格里 1 格的真差异吞成"零差异"，我还把它写成了"逐格同读数"——判据说通过、而它其实什么都没比，与本票第 2、4 处量具缺陷同型，只是这次红在我自己的取证动作里。第三轮双轴审查（Standards 与 Spec 两条轴各自独立）都抓到了它，现按实然改写。被引用的两套 dev 读数不受影响：`--rescore` 的差异集合仍恰好那 4 条、聚合仍 `168/180 = 93.3% -> 172/180 = 95.6%`。
