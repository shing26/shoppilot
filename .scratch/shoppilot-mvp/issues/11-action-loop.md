...
**2026-09-19 追记（票 41）**：本票自陈的"2 轮上限没有针对它的 JVM 内单测"缺口已由票 41 关闭（`GatewayMainPathJvmTest` 四条用例：超限 FALLBACK、预算检查出答案、写动作守卫、多 toolCalls 防御）。同票同时把超限语义对齐 ADR 0008 字面——本票上文 Q&A 中"超限走 FALLBACK 落工单"的口径在票 41 之前与代码的"基于已有事实收尾"实现不一致，票 41 后该口径成为事实；"第 3 次工具请求超出上限"由预算外规划调用判定（模型调用，不是第三个工具轮，与上文第 11 条的纠偏轮同例）。
