...
**2026-09-20 凌晨追记（活体栈恢复后抓到，属本票）**：网关在 dev 口径起栈时**无法启动**——`FeedbackService` 有生产构造器与测试缝构造器两个构造器且无 `@Autowired`，Spring 因"没有唯一构造器"拒绝实例化（`No default constructor found`）。JVM 测试都手工 new，因此没有覆盖启动路径；这是硬闸门恢复活体栈时的第一发现，修复 `9a0052d`（生产构造器加 `@Autowired` 并注明原因）。教训照登：**带测试缝的类必须让 Spring 能唯一确定构造器**，否则单元测试全绿而应用起不来。
