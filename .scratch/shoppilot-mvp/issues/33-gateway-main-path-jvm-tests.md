...
3. "为什么 CI 仍叫 subset？" —— 它只证明干净 runner 能构建并跑 224 条 JVM 测试；SSE 长连接、真实 ES/Qdrant、Ollama、浏览器和压测仍在 17 步活体验收里，名字保留正是为了不把两者混为一谈。（**换代指针（round17）**：224 → 当前 267，17 步 → 当前 22 步；"subset 不冒充全量"这条边界判据本身未变。）
