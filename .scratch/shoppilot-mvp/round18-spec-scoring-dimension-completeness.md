...
- ~~跨项目台账（`D:\WorkBuddyData\`）回写~~ → **2026-09-21 已做**：按计划自己的建议重跑了 `_tools/probe.py`（新证据落 `_raw/ShopPilot.json`，旧基线那份保留为 `_raw/ShopPilot-8d59a44.json`），`_evidence.json`、`ShopPilot/20260921-后端-431790b9.{md,json}`、`汇总-20260921.md`、`ShopPilot-短板清单-20260921.md` 全部按新基线重生成，`项目提升计划-20260921.md` 的 ShopPilot 条目（漂移表、B-3、B-4、执行顺序、复核结论）逐条转「已完成」。**分数落点：97.53% → 100.0%**（B8 2 → 3，12 维全 3 分），依据是『迁移脚本』探针 0 → 7。台账不在本仓版本控制下，改动不影响本仓任何判据。
