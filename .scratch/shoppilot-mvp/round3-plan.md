...
计数口径：共 **11 步**（P0-P10），每步一条「通过判据」，合计 **11 条判据**；机器断言的项数不写死在本行，
...
- **来源**：fixed point `9eede6d`，Standards 轴与 Spec 轴两个子代理并行，分别报 6 条与 5 条。逐条核实后成立 11 条、驳回 3 条（驳回理由见 ticket 20「第三轮收尾」第 10 条）。
...
- **通过判据**：以下六条同时成立才算 Pass。
  ...
  3. `git diff --name-only` 的改动面只含 `README.md`、ticket 20、本计划、`.scratch/shoppilot-mvp/` 下审计脚本与产物；
     `scripts/`、`eval/`、`src/`、`docs/adr/`、阈值常量零改动（由审计 B1-B6 与 F4 钉）。
  ...
  5. 三条驳回项各留机器反证（不是留一句"审查读错了"）：`verify_eval_judge` 的「17 项断言」在盘上不存在（那是
     ...
     而我抄录这句话时被自家 H3 判红一次——记录别人的错引用也会被判成一处引用，这条行为本身照登在本节末）；
