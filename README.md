...
```powershell
# 单元、架构与主链路 JVM 集成测试（3 + 15 + 244 = 262 项）
mvn -o test
...
这条门禁覆盖干净 runner 上的构建与 262 条 JVM 测试：round16 的三条网关主链路 JVM
...
风格票的档位矩阵与注入拼装用例（基座 + 注入段同一次调用发出），不依赖 Docker、Redis、ES、Qdrant 或 Ollama。
