...
```powershell
# 单元、架构与主链路 JVM 集成测试（3 + 12 + 225 = 240 项）
mvn -o test
...
这条门禁覆盖干净 runner 上的构建与 240 条 JVM 测试：round16 的三条网关主链路 JVM
...
升级判据、fail-open、perf 口径）与一条情绪升级集成用例，不依赖 Docker、Redis、ES、Qdrant 或 Ollama。
