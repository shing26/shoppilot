...
拉起 `ollama serve`，等到 11434 之后再 `list`/`pull`。机制可验：同法起一个活 60 秒的 `powershell Start-Sleep 60`，2 秒后它 `alive=True`、父进程 `WmiPrvSE.exe`，
...
`stack` 89s 对上一轮 70s、`polarity` 36s 对 27s，本轮没有可归因的环境变化，差额没查，也不拿它当判据。
上一轮（12:24-12:33，`logs/acceptance-run-20260911-123307.log`）那条环境例外继续照登、一字不改：开跑前把 `TMP`/`TEMP` 指到 `D:\tmp`——
